...
<div class="clase2">
	<div id='wrapper'>
		<div id='instructor'><h3><?=$row['em_nombres']?> <?=$row['em_apaterno']?> <?=$row['em_amaterno']?></div>
...
<?php 
$query = "SELECT asi.as_nombre FROM grupo AS det
INNER JOIN asistentes AS asi ON det.id_grupo = asi.id_grupo WHERE det.id_grupo = '$grupo' AND asi.estatus LIKE 'ACTIVO'";
$result = mysql_query($query);
while($row = mysql_fetch_array($result)): ?>
	<div class="clase2">
		<div id='wrapper'>
			<div id='cliente'><h3><?=$row['as_nombre']?></div>
	     	<img id="constancia" src="formatos/constancia_otro.jpg"/ height="950px;" width="700px;">
		</div>
	</div>
<?php endwhile; ?>
</body>
