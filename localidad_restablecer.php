...
$('#reg').fadeOut();
   ...
   <?php }
  ?>
          <form id='form_reset' method='POST' action='localidad_restablecer.php'>
          <input type="hidden" name="i" value="<?php echo "$localidad";  ?>"/>
            <div class="col-md-12">
              <h3>Datos de la Localidad</h3>
              <div class="col-md-4">
                <label for='inRol'>ID</label>
                <input type="text" class="form-control"  name="id" value="<?php echo "$localidad";  ?>" placeholder="Estado"/>
                <br/>
              ...
              <div class="col-md-4">
                <label for='inRol'>Municipio</label>
                <br/>
                <select name="municipio" class="select">
                 <?php while ($row=mysql_fetch_array($result)){
                  echo "<option value=\"".$row['id_municipio']."\">".$row['mp_nombre'];
                  }
                  ?>  
               </select>
                <br>
              ...
              <input id='btn_submit' class='btn btn-warning' type="submit" name="submit"  value="Restablecer">
               <button class="btn btn-default" type='submit' value='Regresar' formaction="localidad_general.php"/>Regresar</button>
            </div>
          </form>
          </div>
        </div>
